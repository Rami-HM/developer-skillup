...
class Counter2 extends PureComponent{
    ...
    handleClick = (e)=>{
        ...
        // 클릭 된 버튼의 ID 
        var eventBtnId = e.target.id;
        const {resultNum,inputNum} = this.state;
        ...
        if(eventBtnId === "addButton" || eventBtnId === "subButton"){
            if(inputNum === '' || inputNum % 1 !== 0){
                ...
                const num = (eventBtnId === 'addButton') ? 
                    Number(resultNum) + Number(inputNum) : resultNum - inputNum;
                // 컨트롤 발생 시 redo 결과 들은 다 사라짐 
                resultNums.splice(doIdx + 1,resultNums.length - doIdx);
                resultNums.push(num);
                doIdx = resultNums.length -1 ; // undo, redu idx 초기화
                this.setState({resultNum : num});
            }
        }
        else{
            doIdx = (eventBtnId === 'undoButton') ? doIdx - 1 : doIdx + 1;
            this.setState({resultNum : resultNums[doIdx]});
        }
        
        this.setState({
            isFirstIdx : (doIdx === 0) ? true : false,
            isLastIdx : (doIdx === resultNums.length -1) ? true : false
        });
    }
    ...
    render(){
        ...
        const {resultNum,inputNum,isFirstIdx,isLastIdx} = this.state;
        return(
            <div>
            <div className="container">
            <div id="valuebox" className="counter">{resultNum}</div>
            <input id="inputbox" className="input" value = {inputNum || ''} onChange = {onInputNum} />
            <div className="btnGroup">
                <button id="undoButton" className="btn" disabled = {isFirstIdx} onClick = {this.handleClick}>Undo</button>
                <button id="addButton" className="btn" onClick = {this.handleClick}>+</button>
                <button id="subButton" className="btn" onClick = {this.handleClick}>-</button>
                <button id="redoButton" className="btn" disabled = {isLastIdx} onClick = {this.handleClick}>Redo</button>
            </div>
            </div>
        </div>
        )
