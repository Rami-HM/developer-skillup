...
function handleClick(event) {
  ...
  if(eventBtnId == "addButton" || eventBtnId == "subButton"){
    if(inNum == '' || inNum % 1 !== 0){
      ...
      resultNum = (eventBtnId == 'addButton') ? 
      ((resultNum * 1 ) + (inNum * 1)) : ((resultNum * 1 ) - (inNum * 1));
  ...
  if(doIdx != 0){  // 결과 값 리스트에 값이 들어 있을 경우 undo 활성화
  ...
  if(doIdx != resultNums.length -1 ){ // undo 클릭 시 redo 활성화
    document.getElementById("redoButton").removeAttribute("disabled");
  }
  else{
    document.getElementById("redoButton").setAttribute("disabled","disabled");
  }
}
